...
console.log('takes in a string and returns the same string by mapping then demapping it');
...
function arr_identity(str) {
  return map(demap(str));
};
// in-place documentation
console.groupCollapsed('%c arr_identity(arr[charcodes]) -> arr[charcodes]', 'font-weight:bold');
console.log('takes in an arr[charcodes] and returns the same arr after demapping & mapping it');
